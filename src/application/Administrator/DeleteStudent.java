...
public class DeleteStudent extends Application{
	...
	public void start(Stage stage){
	    ...
	    gridPane.setVgap(5); 
	    gridPane.setHgap(5);
		TextField delStudent = new TextField();
		delStudent.setPromptText("Enter Student ID");
		gridPane.add(delStudent, 0,0);
		Button deltStudent = new Button("Delete Student");
		gridPane.add(deltStudent, 1, 0);   
		...
		deltStudent.setOnAction(new EventHandler() {
			@Override
			public void handle(Event event) {
				try{
					if(delStudent.getText().trim().length()>0){
						JdbcDao dao = new JdbcDao();
						boolean flag = dao.deleteUser(delStudent.getText().trim());
						boolean flag2 = dao.deleteStudentFromCourse(delStudent.getText().trim());
						if(flag && flag2){
							Admin1.showAlert(Alert.AlertType.INFORMATION, null, "Form Error!", "Records Added to Course Master Su	ccessfully !!");
						}
					}
				} catch (Exception exp) {
					exp.printStackTrace();
